...
\begin{document}
	...
	\begin{longtable}[c]{| c | c | c | c |}
		...
		$f$ & 0.05 &$\eta_2$ & 0.6 \\
		$r$ & 0.14 & $\beta_1$ & 0.1\\
		$\varepsilon$ & 0.2 & $\beta_4$ & 0.05\\
		$\zeta_1$ & 0.021 & $\zeta_5$ & 0.5 \\
		$\eta_1$ & 0.1977 & $\eta_5$ & 0.45\\
		$\beta_3$ & 0.1428 & $\beta_2$ & 0.25\\
		$\zeta_3$ & 0.003 & $\zeta_4$ & 0.05\\
		$\eta_3$ & 0.057 & $\eta_4$ & 0.01\\
		\hline
		
	\end{longtable}

	We decided to use Germany as an example for modeling epidemics. Data sources links provided at the end of article. Primarily, we relied on dataset by \textit{Our World in Data}. Rates were calculated using correlation with recent observations, research news and publications. These values were actual on April 5, 2020. We take into account further statistics to fine tune the parameters to real life behaviour.
